...
class RentView(View):
    def get(self,request):
        ...
        if location:
            items = Listing.objects.filter(is_available=True,location__icontains=location).filter(Q(item_name__icontains=category) | Q(category__icontains=category)).order_by('-id')
        else:
